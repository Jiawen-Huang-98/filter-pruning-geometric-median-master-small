...
def basic(layer, layer_index, channel, width, prune_rate):
    ...
    less3 = channel[3] * width[3] * width[3] * 9 * channel[3] * (prune_rate) - channel[3] * width[3] * width[3] * 9 * \
            channel[2] * (prune_rate)
    flop = flop - less1 - less2-less3
    return flop
...
def cal(func, layer, rate):
    print("浮点运算量下降百分比：", 1 - int(func(layer, rate)) / int(func(layer, 1)))
...
if __name__ == '__main__':
    ...
    # imagenet_flop(18, 0.7)
    cal(imagenet_flop, 18, 0.7)
    # imagenet_flop(34, 0.7)
    cal(imagenet_flop, 34, 0.7)
    # imagenet_flop(50, 0.7)
    cal(imagenet_flop, 50, 0.7)
