...
from utils import convert_secs2time, time_string, time_file_str, timing, RecorderMeter
...
parser.add_argument('--pretrain_path',default = '',type = str,help = 'the pretrian path')
# compress rate
...
parser.add_argument('--lr_adjust', type=int, default=60, help='number of epochs that change learning rate')
parser.add_argument('--VGG_pruned_style', choices=["CP_5x", "Thinet_conv"],
                    help='number of epochs that change learning rate')

args = parser.parse_args(['data/cifar.python','-a','resnet56','--use_pretrain',
                          '--pretrain_path','best_model_test/model_best_56.pth.tar',
                          '--save_dir','./snapshots/resnet56-{}-rate-0.7'.format(time.strftime('%m-%d_%H-%M')),
                          '--layer_end','162','-b','32'])
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # 使用线性退火方式
        m.decay_rate = max(0.0, float("%.4f" % (1-(2*epoch+1)/args.epochs)))
        print_log('the decay rate now is :{}'.format(m.decay_rate),log)
...
class Mask:
    ...
    def get_filter_codebook(self, weight_torch, compress_rate, length):
        ...
        elif len(weight_torch.size()) == 2:  # 若为全连接层， 则视为1*1卷积进行操作
    ...
    def get_filter_similar_old(self, weight_torch, compress_rate, distance_rate, length):
        ...
        if len(weight_torch.size()) == 4:
            ...
            similar_matrix = torch.zeros((len(filter_large_index), len(filter_large_index)))  # 构建卷积核相似冗余度矩阵
            for x1, x2 in enumerate(filter_large_index):
                for y1, y2 in enumerate(filter_large_index):
                    ...
                    pdist = torch.nn.PairwiseDistance(p=2)  # 计算向量之间的距离，用P范数
            ...
            for x in range(0, len(similar_index_for_filter)):
                codebook[similar_index_for_filter[x] * kernel_length: (similar_index_for_filter[x] + 1) * kernel_length]\
                    = 0
    ...
    def get_filter_similar(self, weight_torch, compress_rate, distance_rate, length):
        ...
        if len(weight_torch.size()) == 4:
            ...
            # 对指定的维度0进行切片，保留weight_vec的第一个维度长度不变，切片至于indices维度相同
            weight_vec_after_norm = torch.index_select(weight_vec, 0, indices).cpu().numpy()
            # for euclidean distance
            similar_matrix = distance.cdist(weight_vec_after_norm, weight_vec_after_norm, 'euclidean')  # 计算两组向量之间的欧几里得距离（一行是一个向量）
            # for cos similarity
    ...
    def init_rate(self, rate_norm_per_layer, rate_dist_per_layer):
        if 'vgg' in args.arch:
            ...
            cfg_index = 0
            previous_cfg = True
            for index, item in enumerate(self.model.named_parameters()):  # 会返回model的名字和参数
