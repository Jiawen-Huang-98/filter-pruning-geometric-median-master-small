...
args = parser.parse_args(['./data/cifar.python', '--dataset', 'cifar10', '--arch', 'resnet32',
                          '--save_path', './logs/best_model_test/', '--epochs', '800', '--learning_rate', '0.1',
...
def adjust_learning_rate(optimizer, epoch, gammas, schedule):
    ...
    for para_group in optimizer.param_groups:
        para_group['lr'] = args.learning_rate
    ...
    def do_mask(self):
        for index, item in enumerate(self.model.parameters()):
            if index in self.mask_index:
                ...
                b = a * self.mat[index]
                for i in range(len(self.mat[index])):
                    if self.mat[index][i] != 1 and abs(b[i]) < 1e-12:
                        b[i] = 0
                        # if abs(b[i]) < 1e-12:

                        # kernel_length = item.data.size()[1] * item.data.size()[2] * item.data.size()[3]
                        # b[abs(b[i * kernel_length: (i + 1) * kernel_length]<1e-12)] = 0
                        # b[abs(b[i])<1e-12]=0
                        # b[abs(b)<1e-8] = 0
